...
| Registry→TypeScript API client generator (`ui/src/`'s "generated API client only," 17 §4.3 rule 10 / 18 §3's M6 gate) — transport and schema-authority rulings decided; only the generator/pipeline itself remains RESERVED | Pydantic schemas exist in `registry_json()` output (i.e. ADR-009 Part B implemented in code, not merely decided) | Discovered scoping `ui/shell/`, 2026-07-30 session; ADR-009 (2026-07-31) ratifies stdlib `http.server` as transport and Pydantic-at-the-registry-layer as schema authority — `12_API` §16's `registry_json()` (`src/kang/api/registry/`) is the source to generate from once populated; the generator tool/pipeline itself remains unscoped |
