...
| De-duplication of repeated consequential-gate calls sharing an idempotency key (found in ADR-021: `dispatch.py::_execute`'s `_store_idempotent` only caches success responses, so a retried `job.disable`-style call creates a second `held_action` row rather than replaying the first — no data-safety impact, but a real user-trust gap: the approval queue can show duplicate pending entries for one intent, and they do not self-resolve since `HeldActionStore.expire_due()`'s 24h sweep also has no scheduled caller yet) | A second consequential gate operation ships (proving the gap generalizes past `job.disable`/`.enable`) or the approval queue sees real duplicate-row use in practice — whichever comes first; fix is either dispatcher-level caching of error responses for idempotency-keyed commands (a real API-004 semantics question, not this ADR's to decide alone) or a gate-side "is there already a pending row for this operation+params" check | ADR-021 Amendment 2026-08-13; `api/dispatch.py::_execute`, `api/operations/consequential.py::require_confirmation` |
| `held_action.*` event registration + publication (ADR-024's D3 slice, decided-not-yet rather than left pending) | A real consumer exists — either the event-channel client binding lands (`api/http_binding.py:9`'s own "M5+" note) and a UI surface needs the approval queue to update live rather than by polling (09_UI §7's "they expire visibly, never silently"), OR M7 builds ADR-001's agent-initiated suspend/resume path, where this event is how the resume fires. M7 merely existing does NOT meet the trigger — the client-initiated path that is built needs no event. Category/grade for `held_action.approved` are pre-decided (Lifecycle, recovery-grade — 15_EVENT_BUS §6.1/:180, EB-003/:82); `.cancelled`/`.expired` grades are genuinely open. Expect the crash-kill test extension (a fifth boundary in `_approve_transactional`'s ghost-event window) to be the expensive part, not the registration | ADR-026 (2026-08-17); ADR-021/ADR-022 both declined the same thing for the same reason; `kernel/bus/event_registry.py` |
...
| Registry→TypeScript API client generator (`ui/src/`'s "generated API client only," 17 §4.3 rule 10 / 18 §3's M6 gate) — transport, schema-authority, and tool-choice rulings all now scoped; awaiting Kang's acceptance of ADR-011, then implementation | Trigger satisfied 2026-08-01: `registry_json()` now serves real JSON Schema for 7 operations (ADR-010, landed) | Discovered scoping `ui/shell/`, 2026-07-30 session; ADR-009 ratified stdlib `http.server` (transport) and Pydantic-at-the-registry-layer (schema authority); ADR-010 implemented the schemas; ADR-011 (proposed, 2026-08-01) picks `json-schema-to-typescript` as the generator tool — this row retires once ADR-011 is accepted and lands |
