...
**Affected documents:** `04_ARCHITECTURE.md`, `05_AGENTS.md` §12 (AG-008), `07_DATABASE.md` §5.5 (`model_call`), `08_PLUGIN_SYSTEM.md` (`sdk.models`) all already described the destination and needed no correction, only implementation; `tests/suites/replay/test_simulated_week.py` needed one (its own proxy for 18 §7.6 went stale the moment `model_call` legitimately started existing — see Verification)
...
**Implemented and verified (2026-09-14), same day as acceptance.** What landed: `domain/ports/model_provider.py` (`TaskSpec`, `ModelResult`, `ModelProvider`, the three typed adapter-level errors), `domain/ports/model_call.py` (`ModelCall`, `ModelCallStore`), `domain/ports/provider_config.py` (`ProviderEntry`, `ProvidersConfig`, including the `[budget]` fields — parsed, not enforced); `adapters/config/providers_loader.py` (fail-closed, mirroring `permissions_loader.py`); `adapters/fakes/model_provider.py` (`FakeModelProvider`, `FakeModelCallStore`); `adapters/sqlite/model_call_store.py`; `kernel/router/router.py` (`Router`); migration `0018_model_call.sql`; `config/defaults/providers.toml` (routes `deep_reasoning`/`routine`/`classification` to `anthropic`, no `private` chain — see D4); the `anthropic` dependency (D6).

**A deviation from D4's own text, found and fixed while writing the migration, not assumed:** `07_DATABASE.md`'s own `model_call` schema includes `invocation_id TEXT REFERENCES agent_invocation(id)`. `agent_invocation` is not yet migrated (M7's own table, correctly out of this ADR's scope) — and confirmed empirically before writing the migration, not assumed: `PRAGMA foreign_keys=ON` (every real connection) makes SQLite refuse even a NULL-valued insert against a `REFERENCES` clause naming a table that does not exist. The column is omitted from migration 0018, to return with its FK in the migration that creates `agent_invocation` — additive, this project's own migration discipline, not a redesign.

**A real, unrelated test failure surfaced and fixed, not routed around:** `tests/suites/replay/test_simulated_week.py::test_the_week_makes_zero_model_calls` asserted the `model_call` table's absence as its proxy for "18 §7.6: M0-M6 contain no model call." Migration 0018 makes that proxy wrong (the table now legitimately exists) without making the underlying claim false (`Router` is not wired into `Core`/`composition.py` — D1's own scope cut, confirmed unchanged by `test_no_model_calls_are_possible`'s own source-grep, which still passes verbatim). Fixed to assert the table is *empty* instead of absent — the claim the test actually owns, correctly proven either way.

Proven, not assumed: `unit/kang/kernel/router/test_router.py` (11 tests — single-provider routing; `ProviderUnavailable` genuinely falls over, both providers tried; `ProviderRefused` propagates without trying a second provider; every candidate failing propagates the *last* provider's own error; an empty/filtered chain raises `NoProviderAvailable` with nothing logged; `privacy_tier="private"` fails closed with the cloud provider never called, and routes correctly to a configured `local_only` provider; an unwired `providers.toml` entry is skipped, not a crash; the circuit breaker opens at its configured threshold, stays open through the cooldown via `FakeClock.advance()` — no real sleep, no network — and closes after it elapses; a success resets the consecutive-failure count) + `unit/kang/adapters/config/test_providers_loader.py` (10 tests, including the shipped default loading for real) + `integration/sqlite/test_model_call_store.py` (4 tests, including the CHECK constraint on `outcome`). Live-verified against a real throwaway `%KANG_HOME%`: migration 0018 applies cleanly on a real connection; the shipped `providers.toml` loads for real; a real `Router.route()` call against a real `SqliteModelCallStore` writes a real, readable `model_call` row. Full suite: 961 passed (up from 935 — the 25 new tests plus the one fixed assertion, no silent coverage loss). Full lint suite: 0 hard violations, `ruff check .`/`lint-imports`/banned-patterns/tree-hygiene all clean. Zero live network calls anywhere in the test suite (13_TESTING §1).
