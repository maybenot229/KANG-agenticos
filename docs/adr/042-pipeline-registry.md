...
- **`AgentDefinition.pipelines`'s own membership claims remain unvalidated against real pipeline step lists this slice** — named as a candidate follow-up, not silently assumed correct. **Built same-day, once real pipeline data existed to prove it against** — see Verification's second paragraph: `check_pipeline_membership_reciprocity` now exists, is tested, and its manual equivalent (an eyeball check made while grounding this ADR) already caught a real gap in ADR-040's own shipped data.
...
**Follow-up completed same day:** `check_pipeline_membership_reciprocity` (`kernel/orchestrator/pipeline_registry.py`) — the bidirectional check this ADR's own Consequences section named as a candidate, not built blind, until real pipeline data existed to prove it against. Every agent's own `pipelines` claim now checked for exact set equality against the pipelines that actually list it as a step; a false claim or a missing one both refuse (fail-closed, same posture as everything else in this file). 4 new tests: matching claims pass; a false claim raises; a missing claim raises; the real shipped catalog and real shipped pipelines reciprocate cleanly end to end. Full suite re-confirmed green at **1038 passed**; lints re-confirmed 0 hard violations, 8/8 import contracts kept.
