...
| [031](031-daily-backup-job.md) | KANG has never taken a backup: wiring `backup.snapshot` as a fourth automatic job | **proposed** | 05_AGENTS App. E (2 corrections), 12_API §16, `config/defaults/permissions.toml`, `kernel/runtime/scheduler_wiring.py` |
| [032](032-backup-verify-job.md) | `backup.verify`: closing the "nonexistent until restore-tested" gap ADR-031 left open | **proposed** | 07_DATABASE Part XII.3 (2 interpretive decisions), 12_API §16, `kernel/runtime/scheduler_wiring.py` |
| [033](033-system-health-backup-status.md) | `system.health` exposes backup age + last restore-verification result | **proposed** | 09_UI §12, 12_API §16, `api/operations/system_ops.py`, `api/schemas/system.py` |
| [034](034-offmachine-backup-warning.md) | The off-machine backup warning (07 Part XII.5): a Kang-configured marker, a weekly check, an `attention` notification | **proposed** | 07_DATABASE Part XII.5, 09_UI §12 (doc gap), 05_AGENTS App. E (doc gap), 15_EVENT_BUS §6.1, `config/defaults/{kang,permissions}.toml`, `kernel/runtime/scheduler_wiring.py` |
