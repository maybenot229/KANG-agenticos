...
- **Every existing store method, handler, and test is unchanged.** A "command" operation's entire handler invocation — the handler function, whatever store calls it makes, whatever it publishes on the bus — is submitted as *one atomic unit of work* to the write-executor. A "query" operation's handler invocation is submitted to the read pool. Nothing inside either unit of work needs to know it is running this way; it is exactly the same synchronous call graph as today, just executing on a dedicated worker instead of the main thread.
...
- `Dispatcher` gains one new async entry point. Its body is unchanged in substance — it reads `entry["kind"]` (already there) and submits the existing, unmodified `_run(...)` call to the write-executor (`command`) or read-pool (`query`) from D3, `await`-ing the result.
...
- **Any change to the 18 stores' or 15 handlers' own tests.** D1's whole point is that none are required. Only D2's and D3's own new code gets new tests; D4's tests cover the routing seam (command → executor, query → pool) and the `aiohttp` binding itself.
...
- **18 stores, 19 fakes, 15 handlers, and ~900 existing tests are untouched.** This is the point of D1, not an oversight — stated plainly so a future session does not "finish the job" by converting them without a new, specific reason (the same discipline ADR-029 D4 used for the UI's "task card" label).
