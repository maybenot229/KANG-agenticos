...
**Implemented and verified (2026-09-16), same day as acceptance.** What landed: `agents/runtime/executor.py` (`Dispatch`, `ExecutorDeps`, `AgentRunResult`, `run_mechanical_agent`); two new exceptions on `domain/ports/agent_definition.py` (`ToolNotAllowed`, `CognitiveAgentNotSupported`); the `deadline_sweep.toml` correction (see ADR-040's own updated Verification note).

**A layering consequence found while implementing, not anticipated in the draft:** `kang.agents` is forbidden from importing `kang.api` by `tools/importlinter.toml`'s existing contract — unconditionally, not merely "pre-M7" (the contract's own name is a label, not a carve-out written into its rule). `Dispatch` is therefore a plain, generic `Callable[[str, dict, str, str | None], dict]` — never `ApiRequest`/`Dispatcher` themselves — mirroring `api/http_binding.py`'s own "stays ignorant of HOW a request reaches the Core" pattern exactly. Confirmed, not assumed: `lint-imports` reports all 8 contracts kept with `executor.py` in the tree.

**A second implementation finding:** `run_mechanical_agent`'s natural parameter count (agent, operation, params, dispatch, sessions, new_id, clock, idempotency_key) is 8 — over the size lint's 6-parameter hard limit. `dispatch`/`sessions`/`new_id`/`clock` bundle into a new `ExecutorDeps` dataclass (11 §4's own "then a dataclass" rule), bringing the callable's own subject-vs-collaborator split in line with `DispatcherDeps`'s already-established shape.

Proven, not assumed: `unit/kang/agents/runtime/test_executor.py` (5 tests, against a recording fake `Dispatch` + the real `FakeSessionStore`): a cognitive agent is refused before any dispatch or session mint; an operation outside the allowlist is refused before any dispatch; a real dispatch mints a session for `agent:deadline_sweep`/`first_party=False`/the injected clock's own timestamp, and returns the response verbatim; the idempotency key is optional and passed through as `None` when omitted; two runs mint two distinct session tokens, never reused. Live-verified against a real throwaway `%KANG_HOME%` with a throwaway `permissions.toml` granting `agent:deadline_sweep = ["deadlines.mark_alerted"]` (never the shipped default, confirmed empty of any `agent:` grant afterward): a real `deadline.create` followed by a real executor-driven `deadline.sweep` genuinely transitions and alerts the deadline it created — `{"alerted": [...], "count": 1}`, not a no-op. Throwaway home deleted after. Full suite: 1017 passed (up from 1012, no silent coverage loss). Full lint suite: 0 hard violations (`run_mechanical_agent`'s own parameter count needed the `ExecutorDeps` split), `ruff check .`/`lint-imports`/banned-patterns/tree-hygiene all clean. Zero network, zero model calls anywhere (13_TESTING §1) — a deterministic, zero-model agent shape by construction.
