...
**Corrected by ADR-042** (2026-09-16): D4's own transcription left `notifier.pipelines` and `memory_steward.pipelines` both `[]`, even though Appendix A's own pipeline list (`docs/05_AGENTS.md:435`) names both as real steps — `notifier` closes `competition_intake` and `weekly_close`; `memory_steward` performs `weekly_close`'s own weekly-consolidation step. Undiscoverable at D4's own implementation time (no real pipeline data existed yet to check a `pipelines` claim against — ADR-042 D3 names this exact reciprocal check as a real one, deliberately not built blind until real pipeline data existed to prove it against). Corrected to `notifier.pipelines = ["competition_intake", "weekly_close"]` and `memory_steward.pipelines = ["weekly_close"]` once ADR-042 shipped that data. No other shipped definition's `pipelines` field was found wrong by the same check (`competition_scout`, `competition_strategist`, `critic`, `planner`, `researcher` already matched their real pipeline memberships exactly).
