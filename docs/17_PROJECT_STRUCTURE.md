...
**Corrected by ADR-035** (2026-09-13): stdlib `http.server` is superseded by `aiohttp` — 11_CODING §12's "asyncio single-loop in the core" rules out `http.server`'s own blocking `serve_forever()` loop once the Core actually moves to that model (ADR-030's execution model, ADR-036's slice plan). This row named the destination ahead of the code at the time; `http_binding.py` itself now genuinely runs on `aiohttp` (ADR-036 D4, landed 2026-09-14 — code and doc now agree, not merely the doc anticipating the code). Same document-consistency-correction shape as ADR-009's own row fix above, applied here rather than re-litigated as a separate ADR.
...
**Composition root exception (the one place wiring is legal).** `src/kang/kernel/runtime/composition.py` (the startup assembler) MAY import adapters and everything else, because *something* must instantiate concretions and inject them. Two more files carry the identical exemption, each split out of `composition.py` when its own slice pushed that file past the size lint's hard limits — `scheduler_wiring.py` (ADR-023) and `query_routing.py` (ADR-037) — one conceptual composition root across three files, not three roles. These three are the only files in `src/` exempt from rows above; each contains wiring only (lint: no logic, no branching beyond config), and each is named here, and by exact module in `tools/importlinter.toml`, so the exemption cannot spread further without its own line and its own justification.
...
| **New agent** | `agents/definitions/{name}/` (TOML + prompts) + grants + optional pipeline entries (05 §17: "no kernel changes"). Dozens of agents = dozens of data folders; at >20, group `definitions/{domain}/{name}/` — a move of *data*, no import changes, no ADR | Never |
